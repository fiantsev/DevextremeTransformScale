...
var getRealElementWidth = function(element) {
    ...
    if (width > 0) {
        return window.DevExtremeIntegration.ApplyDashboardScaleCorrection(width); //WAS: return width
    } else {
        return window.DevExtremeIntegration.ApplyDashboardScaleCorrection(element.offsetWidth); //WAS: return element.offsetWidth
    }
};
...
exports.AreaItem = Class.inherit({
    ...
    _getRowHeight: function(index) {
        ...
        if (row && row.lastChild) {
            ...
            if (height > 0) {
                return DevExtremeIntegration.ApplyDashboardScaleCorrection(height); //WAS: return height
            } else {
                return DevExtremeIntegration.ApplyDashboardScaleCorrection(row.offsetHeight); //WAS: return row.offsetHeight
            }
        }
        return 0
    },
